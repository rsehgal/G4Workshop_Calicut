...
const double sigma_meas = 0.5; //0.01;  // 0.01 cm resolution
...
public:
    ...
    void Predict() {
        ...
        std::cout<<"cov(1,1)"<<cov(1,1)<<std::endl;
    }
    ...
    void Update(const Hit &hit) {
        ...
        std::cout << "Kalman Gain: " << K(0,0) << ", " << K(1,0) << std::endl;
...
int main(int argc, char **argv) {
    ...
    if (!file || file->IsZombie()) {
        std::cerr << "Error opening file tracking1muon.root" << std::endl;
        return 1;
    }
   ...
   // TH1D *hPosX = new TH1D("hPosX", "Position X Distribution; X (cm); Events", 100, -1000., 1000.);
    TH1D *hPosX = new TH1D("hPosX", " ActualX - SmearedX; X (cm); Events", 200, -1000., 1000.);
  ...
  TH1D *hMomentumResidual = new TH1D("xpositionResidual", "ActualX-KalmanX", 200, -1000, 1000);
  ...
  hMomentumResidual->SetLineWidth(2);
    ...
    for (auto &entry : eventHits) {
        ...
        for (size_t i = 1; i < hits.size(); i++) {
          ...
          //hMomentumResidual->Fill(kalman_x);
          
          std::cout<<"XPos=\t"<<true_x<<"\tsmeared_x=\t"<<smeared_x<<"\tkal_x=\t"<<kalman_x<<"\tresidue=\t"<<true_x - kalman_x<<"\tXpos-smeared_x=\t"<<true_x-smeared_x<<"\n";
